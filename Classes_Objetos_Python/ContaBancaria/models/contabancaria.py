...
class ContaBancaria:
    ...
    def transferir(self, transferencia, destinatario):
        '''
        Objetivo: Método que realiza a transferência de um valor entre contas bancárias.
        Entrada: valor (float), destinatário (string)
        Return: True se a operação obtiver sucesso. False se a operação não for realizada.
        '''
        if transferencia <= self.saldo:
            ...
            self.historico.append({"operacao": 2,
                                   ...
                                   "saldo": self.saldo,
                                   "dataetempo": int(time.time())})
            print(f"Transferência de R${transferencia} realizada com sucesso!")
            return True
        else:
           a = input(f"Deseja usar o limite de ({self.limite}) para sacar (R${transferencia}) ? (s para sim)")
           if a == 's':
                if (self.saldo + self.limite) >= transferencia:
                    self.saldo -= transferencia
                    print(f"Transferência de R${transferencia} realizado com sucesso!")
                    return True
                else:
